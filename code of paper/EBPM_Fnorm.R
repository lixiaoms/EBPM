## Frobenius norm minimization version
...
sigma_semipositive<-function(cov_input){
  ...
  ## Optimatization by minimize the Frobenius norm--------------------
  cov <- cov_input[!index,!index]
  A <- eigen(cov)
  sigma_me2 <- A$vectors%*%diag(pmax(A$values,0))%*%t(A$vectors)
...
EB_denoise <- function(Y,n=1,k_max=10){
...
EB_complete <- function(Y,obs,iter=3,n=1,k_max=10){
    moment <- moment_int(Y,obs)
    cov_moment <- sigma_semipositive(moment$sigmahat)
    X_hat <- denoise_map(moment$mu,cov_moment,Y,obs,n,k_max)
    if (iter<=0){
